...
base_url = "https://checamos.afp.com/list/all/all/all/38559/49?page="

# Define the range of pages you want to scrape
start_page = 1
end_page = 85  # Change this to the last page you want to scrape
...
for page_number in range(start_page, end_page + 1):
    ...
    if response.status_code == 200:
        ...
        # Translate the text within the <html> tag (the entire page)
        html_tag = soup.find('html')
        if html_tag:
            translated_html = translator.translate(str(html_tag), src='auto', dest='en')
            soup = BeautifulSoup(translated_html.text, 'html.parser')
    ...
    for i in range(min(len(card_divs), len(h2_elements))):
        ...
        for link in links:
            href = link.get('href')
            abs_href = 'https://checamos.afp.com' + href # get the absolute path ...IMPORTANT CHANGE FOR LATIN AMERICA
...
csv_file = 'data_latin_america.csv'
